...
function Reports() {
  ...
  const rawList = reports.length > 0 ? reports : mockReports
  const list = rawList.filter(r => {
    if (tab === 'Sample Reports') return r.type === 'Sample Report' || r.type === 'Sample' || !r.type
    return r.type === 'Batch Report' || r.type === 'Batch'
  })
...
function Analytics() {
  ...
  const scores = trendData[range] || trendData['30 days']
  const avgScore = analytics?.averageScore || Math.round(scores.reduce((a, b) => a + b, 0) / scores.length)
  ...
  const distData = analytics?.feedTypeDistribution || { 'Maize Silage': 3, 'Cattle Feed Pellet': 1, 'Grass Silage': 1, 'Dairy Concentrate': 1 }
